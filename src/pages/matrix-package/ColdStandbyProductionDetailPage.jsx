...
const FRONTEND_ENV_SECTIONS = [
  {
      ...
      fields: FRONTEND_FIELD_DEFINITIONS.map((field) => ({
        ...
        generatedFallback: field.generatedFallback,
      })),
  },
  {
      key: 'test',
...
const FRONTEND_GENERATED_FALLBACK_FIELDS = new Set(
  FRONTEND_ENV_SECTIONS
    .flatMap((section) => section.fields)
    .filter((field) => field.generatedFallback)
...
function buildGeneratedEnvironmentUrlValues(packageDetail) {
  return {
    ...buildGeneratedH5Values(packageDetail),
    ...buildGeneratedTrackingValues(packageDetail),
  }
}

function normalizeGeneratedEnvironmentUrlOverride(fieldName, value, packageDetail) {
  const normalizedValue = String(value || '').trim()
  const generatedValue = buildGeneratedEnvironmentUrlValues(packageDetail)[fieldName] || ''
  return normalizedValue && normalizedValue !== generatedValue ? normalizedValue : ''
}
...
function buildNoteFormValues(notes, packageDetail = null) {
  ...
  if (frontendValues && typeof frontendValues === 'object' && !Array.isArray(frontendValues)) {
    ...
    Object.entries(buildGeneratedEnvironmentUrlValues(packageDetail)).forEach(([key, value]) => {
      if (!String(frontendValues[key] || '').trim()) frontendValues[key] = value
    })
  }
  return values
}
...
function ColdStandbyProductionDetailPage() {
      ...
      const tasks = Object.entries(pending)
        .map(([noteType, fieldMap]) => {
          ...
          const fields = Object.keys(fieldMap || {}).reduce((acc, fieldName) => {
            acc[fieldName] = noteType === 'FRONTEND' && FRONTEND_GENERATED_FALLBACK_FIELDS.has(fieldName)
              ? normalizeGeneratedEnvironmentUrlOverride(fieldName, sectionValues[fieldName], detail)
              : sectionValues[fieldName]
            return acc
          }, {})
  ...
  }, [applySideNotesResponse, canEdit, collectPendingFieldPatches, detail, form, id])
  ...
  const handleNoteValuesChange = (changedValues, allValues) => {
    ...
    if (nextFrontendValues && typeof nextFrontendValues === 'object') {
      const generatedEnvironmentUrlValues = buildGeneratedEnvironmentUrlValues(detail)
      Object.keys(generatedEnvironmentUrlValues).forEach((fieldName) => {
        if (
          Object.prototype.hasOwnProperty.call(changedValues?.FRONTEND || {}, fieldName)
          && !String(nextFrontendValues[fieldName] || '').trim()
        ) {
          nextFrontendValues[fieldName] = generatedEnvironmentUrlValues[fieldName]
        }
      })
      form.setFieldsValue({ FRONTEND: nextFrontendValues })
      setFrontendNodeValues(nextFrontendValues)
  ...
  const handleFrontendNodeFieldChange = (fieldName, value) => {
    if (!canEdit) return
    const currentValues = form.getFieldsValue(true)
    const generatedValue = buildGeneratedEnvironmentUrlValues(detail)[fieldName] || ''
    const displayValue = FRONTEND_GENERATED_FALLBACK_FIELDS.has(fieldName) && !String(value || '').trim()
      ? generatedValue
      : value
    const nextFrontendValues = {
      ...(currentValues.FRONTEND && typeof currentValues.FRONTEND === 'object' ? currentValues.FRONTEND : {}),
    ...
    form.setFieldsValue({ FRONTEND: nextFrontendValues })
    setFrontendNodeValues(nextFrontendValues)
    scheduleAutoSave(nextValues, { FRONTEND: { [fieldName]: displayValue } })
  }
